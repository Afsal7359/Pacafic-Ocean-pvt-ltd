...
const AddRevenueModal = ({setModal,setCostData,setRevenueData,CostData,RevenueData,item}) => {
  const { register, handleSubmit, setValue, formState: { errors } } = useForm();
    ...
    const handleCheckboxChange = (e) => {
      ...
      const onSubmit = (data) => {
        ...
        // Initialize CostData and RevenueData if they are not already arrays
        if (!Array.isArray(item.CostData)) {
          item.CostData = [];
        }
        if (!Array.isArray(item.RevenueData)) {
          item.RevenueData = [];
        }
      
        if (costItems.Costparty) {
          item.CostData.push(CostArr);
        }
        if (revenueItems.Revenuparty) {
          item.RevenueData.push(RevenueArr);
        }
